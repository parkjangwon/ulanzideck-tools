...
MO_CLEAN_TEMPLATE := actions/mo-clean/ulanzi-action.template.json
MO_CLEAN_ACTION_JSON := actions/mo-clean/ulanzi-action.json
...
.PHONY: help install-obsidian-new-note render-obsidian-action render-mo-clean test-obsidian-new-note show-obsidian-button backup-profile
...
help:
	...
	@echo "  make render-mo-clean            Render mo clean Ulanzi JSON snippet"
...
render-mo-clean:
	@sed "s#__HOME__#$(HOME)#g" "$(MO_CLEAN_TEMPLATE)" > "$(MO_CLEAN_ACTION_JSON)"
	@echo "Rendered: $(MO_CLEAN_ACTION_JSON)"
